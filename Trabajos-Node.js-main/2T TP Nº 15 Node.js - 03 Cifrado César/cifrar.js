...
function recuperar(pedido, respuesta) {
    ...
    pedido.on("end", () => {
        ...
        const pagina =
            ...
            cifrar(formulario["string"], parseInt(formulario["num"])) +
            `</div></body></html>`;
        respuesta.end(pagina);
        console.log(cifrar(parseInt(formulario["num"])));
    });
}

function cifrar(string, n) {

    let string2 = "";
    for (x = 0; x < string.length; x++) {
        let letra2 = String.fromCharCode(string.charCodeAt(x) + n);
        string2 += letra2;
    }
    console.log(string2);
    console.log();
